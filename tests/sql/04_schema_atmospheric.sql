-- Test: verify columns in silver atmospheric_condition table
WITH expected AS (
  SELECT explode(array(
    'ACCIDENT_NO','ATMOSPHERIC_CODE','ATMOSPHERIC_DESC','SEQ','INGESTION_TS','LOAD_TS'
  )) AS column_name
),
actual AS (
  ...
  WHERE lower(table_catalog) = 'yinli_catalog'
    ...
    AND lower(table_name) = 'atmospheric_condition'
)
SELECT
